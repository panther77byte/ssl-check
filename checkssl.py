...
import smtplib
import argparse
import datetime  # Import the entire datetime module now
    ...
    try:
        with socket.create_connection((hostname, port)) as sock:
            with context.wrap_socket(sock, server_hostname=hostname) as ssock:
                ...
                if cert:
                    # Parse expiration date from certificate (assumed to be naive)
                    expiration_date = datetime.datetime.strptime(cert['notAfter'], "%b %d %H:%M:%S %Y %Z")
                    
                    # Extract issuer from certificate (use dictionary method to get 'commonName')
                    issuer = dict(x[0] for x in cert['issuer'])['commonName']
                    
                    # Make expiration_date aware by assigning UTC timezone
...
def check_ssl_certificates(sites, email=None):
    ...
    for site in sites:
        ...
        # Append the result without the issuer name (for the table)
        results.append({
            "No": len(results) + 1,  # Serial number as "No"
...
def send_expiration_notice(email, hostname, expires, days_left, issuer):
    ...
    smtp_user = ""  # Replace with your SMTP email
...
def main():
    ...
    print(tabulate(ssl_results, headers="keys", tablefmt="table"))
